...
#### Step 1: Show that $\mathbb{E}_{\theta}S_{\theta}(X)=0$
...
#### Step 2: Show that LAN holds at $\theta$
...
#### Step 2.1: Analysis for $\sum_{i=1}^{n}W_{i}$

Let $c_{n}=\sqrt{n}\left(\sqrt{p_{\theta+h/\sqrt{n}}}-\sqrt{p_{\theta}}\right)$
...
#### Step 2.2: Analysis for $\sum_{i=1}^{n}W_{i}^{2}$
